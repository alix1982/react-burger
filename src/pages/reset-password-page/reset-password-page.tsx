...
export const ResetPasswordPage = (): React.ReactNode => {
  ...
  const isLoadingResetPassword = useAppSelector(SisLoadingResetPassword);
  const errorResetPassword = useAppSelector(SerrorResetPassword);

  useEffect(() => {
    if (!(localStorage.getItem('isChangePassword') === 'true')) {
