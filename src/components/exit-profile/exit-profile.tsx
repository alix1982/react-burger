import { Preloader } from '@krgaa/react-developer-burger-ui-components';
import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';

import {
  logout,
  SerrorLogout,
  SisLoadingLogout,
  StextLogout,
} from '@/store/authSlice/authSlice';
import { useAppDispatch, useAppSelector } from '@/store/hooksStore';

import styles from './exit-profile.module.css';

export const ExitProfile = (): React.ReactNode => {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const textLogout = useAppSelector(StextLogout);
  const isLoadingLogout = useAppSelector(SisLoadingLogout);
  const errorLogout = useAppSelector(SerrorLogout);

  useEffect(() => {
    dispatch(logout());
  }, []);

  useEffect(() => {
    if (textLogout) {
      const timer = setTimeout(() => {
        navigate('/login');
      }, 3000);
      return (): void => clearTimeout(timer);
    }
  }, [textLogout]);
  return (
    <article>
      {isLoadingLogout ? (
        <Preloader />
      ) : (
        <p
          className={`${styles.exitProfile} ${errorLogout ? 'text text_type_main-medium text_color_error' : 'text text_type_main-medium'}`}
        >
          {errorLogout ? errorLogout : textLogout}
        </p>
      )}
    </article>
  );
};
